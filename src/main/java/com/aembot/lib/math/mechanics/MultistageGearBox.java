...
public class MultistageGearBox {
  ...
  /**
   * Represents and individual "stage" in the gear box, a stage in this context is the linkage
   * between 2 gears
   */
  public static final class Stage {
     ...
     *
     * @param drivingGearTeethCount Number of teeth on the gear the input is applied to
     * @param drivenGearTeethCount Number of teeth on the gear the is being driven * @param name
     *     Name of the gearbox stage
     */
    ...
    /**
     * Retrieve the overall gear ratio between the driving gear (motor) and the driven gear (output
     * shaft or wheel).
     *
     * <p>The gear ratio is defined as:
     *
     * <pre>
     *     Gear Ratio = (Teeth on Driven Gear) / (Teeth on Driving Gear)
     * </pre>
     *
     * A higher gear ratio increases torque and reduces speed, while a lower ratio increases speed
     * and reduces torque.
     *
     * <p>This handles div by 0
     *
   ...
   *
   * <p>The order in which stages are added should match the physical order in which they are
   * encountered starting from the motor output toward the final driven gear.
   *
   * <p>Each stage represents a pair of meshed gears (driving and driven) that contribute to the
   * total gear reduction or increase of the gearbox. The overall gearbox ratio is the product of
   * all individual stage ratios.
   *
   * <p>For example, a two-stage gearbox with a 12→36 and 15→45 gear pair would yield a total
   * reduction of (36/12) × (45/15) = 9:1.
   *
   * <p>Each time a stage is added, the total ratio is automatically recomputed.
   *
  ...
  /**
   * Calculate the output value from the input using the current total ratio. Example: input motor
   * rotations / ratio = output shaft rotations.
   *
  ...
  /**
   * @return current total gearbox ratio (product of all stage ratios). Returns 0 if no stages are
   *     present.
   */
  ...
  /** Recompute the total gearbox ratio as the product of all stage ratios. Safe for empty lists. */
